...
const PlanCard: React.FC<{
  ...
  <div className={`relative p-4 rounded-xl border flex flex-col h-full transition-all ${recommended
...
const App: React.FC = () => {
  ...
  const handleUpgrade = async (plan: PlanLevel) => {
    try {
      ...
      setAppState(AppState.LOADING);
      addNotification(`Redirecting to payment for ${plan}...`, "info");

      try {
        const { createCheckoutSession } = await import('./services/stripeService');
        await createCheckoutSession(plan, user.email);
        // Note: The above usually redirects away.
      } catch (err: any) {
        console.error("Payment init failed:", err);
        addNotification(err.message || "Failed to start payment.", "error");
        setAppState(AppState.IDLE);
      }
    ...
    <div className="h-screen flex flex-col bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans transition-colors duration-200">
      ...
      >
        {notifications.map(n => (
          <div
            key={n.id}
            className={`pointer-events-auto px-4 py-2.5 rounded-md shadow-xl text-white font-medium text-xs tracking-wide animate-in slide-in-from-right fade-in duration-300 max-w-sm border border-white/10 ${n.type === 'success' ? 'bg-slate-900 dark:bg-slate-800' :
